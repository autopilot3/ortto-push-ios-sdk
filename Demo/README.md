...
cp push-token-repro/Configurations/Local.xcconfig.example \
   push-token-repro/Configurations/Local.xcconfig
```

```sh
# 2. Open the workspace
open push-token-repro.xcworkspace
```

**3.** Fill in `push-token-repro/Configurations/Local.xcconfig` (see [Configuration](#configuration)).

**4.** Pick the scheme for your provider — `PushNotificationDemo-APNS` or `PushNotificationDemo-FCM` — and run.
...
| | `PushNotificationDemo-APNS` | `PushNotificationDemo-FCM` |
| --- | --- | --- |
...
- **Initialize** — `Ortto.initialize(appKey:endpoint:)` in `App/PushNotificationDemoApp.swift`.
...
> The app entry file (`PushNotificationDemoApp.swift`) is the map: each touchpoint is marked with an `// Ortto SDK:` comment.
...
Config lives in `push-token-repro/Configurations/Local.xcconfig` (gitignored; copied from `Local.xcconfig.example`). The committed `BuildDefaults.xcconfig` declares these empty, and Xcode expands them into the app `Info.plist` at build time.
...
For the **FCM** target, also drop a real Firebase plist at `push-token-repro/GoogleService-Info.plist` (bundled only when present). Never commit `Local.xcconfig` or `GoogleService-Info.plist`.
...
- Start at `App/PushNotificationDemoApp.swift` (`@main`).
...
| File | Purpose |
| --- | --- |
| `App/PushNotificationDemoApp.swift` | App entry point (`@main`) and `Ortto.initialize` / `OrttoCapture.initialize`. |
...
```sh
xcodebuild -workspace push-token-repro.xcworkspace \
  -scheme PushNotificationDemo-APNS \
  -destination 'generic/platform=iOS Simulator' build

xcodebuild -workspace push-token-repro.xcworkspace \
  -scheme PushNotificationDemo-FCM \
  -destination 'generic/platform=iOS Simulator' build
```
